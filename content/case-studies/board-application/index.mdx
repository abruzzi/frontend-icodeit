...
With data modeling we know how the data looks like, and with API design we know how the data should be fetched from server side. And that is the begining of the frontend data design - state management.

You can think of frontend state management as a sub-system lives under the user interface, it has the source of truth of the current UI: what cards to show, which part of the UI is still pending, etc. And beside these state, there are also APIs - functions that can change the state (usually triggered by user interaction, it also can be system event, like a timer or SSE event listener).

The simplest solution would be a React Context API, it holds a global store object, and the children components listen changes from the store, whenever something changed: a user moved card from column A to B, a user changed it's avatar, another user changed card title from their side, etc. All these change propogate to the children components for a re-render, so UI is consistent.

A typical store implemented with React Context might look like this:
...
We have used falttend version of the board data in frontend, so we can avoid the problem of duplication (the zombie card issue we discssed earlier) and inconsistent user avatars.

Notice that we not ony have these normalised data, but also the APIs like `moveCard`, `upsertUser`, etc. And then we can call these functions inside user action handler or SSE event handler.
