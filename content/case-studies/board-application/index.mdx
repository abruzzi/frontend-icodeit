...
A context API for a **normalised** board might expose both **state** and **mutators** like this:
...
type BoardContextValue = {
  ...
  /** Replace or merge after a snapshot from the server */
  ingestBoard: (data: BoardPayload) => void;
  ingestUsers: (users: User[]) => void;
  upsertUser: (user: User) => void;
  updateCard: (cardId: string, updates: CardFieldUpdates) => void;
  removeCard: (cardId: string) => void;
  addCard: (columnId: string, card: { id: string; title: string }) => void;
  moveCard: (cardId: string, toColumnId: string, afterCardId: string) => void;
};
```

Keeping the board **normalised** in the client (maps by id, column order as id lists) avoids duplicating entities in nested trees—the **stale identity** problem we walked through with the zombie-card demo—and keeps avatars and assignee labels consistent everywhere.
...
A thin consumer might resolve the current card and assignee from the store, then route a picker through `upsertUser` + `updateCard`:
...
function CardAssigneeField({ cardId }: { cardId: string }) {
  ...
  const assignee =
    card?.assigneeId != null ? state.usersById[card.assigneeId] ?? null : null;
  ...
  return (
    <UserSelect selected={assignee} onChange={handleAssigneeChange} />
  );
}
```
...
A coarse lifecycle runs **Capture → Apply → Buffer → Sync → Settle → Reconcile**. The diagram below lays the same steps out as a left-to-right pipeline (pan and zoom if it feels tight on a narrow viewport).

<BoardCommandLifecycleFlow />

A command can be as simple as a typed object like below, note there is a `undo` to record what need to be rollback if the server fails. 
...
### Deep dive: Keyboard & accessibility (the “move menu” pattern)
...
**Recommendation — action menu over accessible drag:** Prefer an **action menu** (sometimes called a **move menu**): e.g. focus a card and press **`M`** (or use a **⋯** menu) to open **“Move to…”** — a dialog or list of columns with clear labels. The user confirms a **destination**; the app sends the same **move operation** the pointer path would have triggered. That pattern is **more reliable** than exposing drag-and-drop as the main accessible path. Still add **roving tabindex** within the board, **live region** announcements when focus or column context changes, and **retries** with exponential backoff on **5xx** / network—**never** mint a **new** `operationId` for the **same** user gesture.
