---
title: "Board Application: Frontend Architecture"
summary: "Normalised board modelling, idempotent moves and command queues, LexoRank-style ordering, and accessible drag-and-drop."
pageSummary: >
  A long-form frontend case study built around a kanban board: nested versus normalised data,
  API sketches (REST snapshot reads, intent-shaped moves, idempotency and 409 reconciliation),
  client replica and command-queue thinking, ordering trade-offs from dense integers through
  LexoRank-style strings, SSE as a transport example, a performance slice on code-splitting and
  lazy card details (plus perceived performance and where virtualisation and caching belong),
  local Pragmatic drag-and-drop demos plus a separate accessibility demo (keyboard path, menu,
  live region), reliability failure modes, and an end-of-article testing rubric. Interactives are
  mostly browser-local teaching aids—no shipped backend walkthrough—and the piece does not compare
  state-management or data-fetching libraries; it stays at shapes, contracts, and how things fail
  under concurrency and bad networks.
lastEdited: "2026-04-08"
kind: "case-study"
status: "published"
scenario: "board-app"
tags:
  ["performance", "reliability", "testing", "state-management", "a11y", "real-time"]
patternRefs:
  - slug: "lexorank-card-ordering"
    reason: "Companion pattern for the ordering section (gaps, rebalance, LexoRank-style strings); page is still a stub alongside this case study."
quickTake:
  - "Reasonable slice to ship first: fixture → working UI → client moves → persist for one user → then realtime and conflicts; the article widens that spine with local demos and design detail."
  - "Normalised entity maps plus ordered card ids per column; stable operationId (or equivalent) on write intents when a server is involved."
  - "Out of scope here: picking Redux vs Zustand, TanStack Query vs SWR—patterns are meant to map onto any stack."
productionNotes:
  - "Repeated duplicate moves, odd ordering, or constant optimistic rollbacks usually point to idempotency, event ordering, or version checks—not isolated UI defects."
  - "Treat an accessible move path (menu/keyboard) as part of v1 alongside drag; pointer-only flows leave many users stuck."
...
<BoardLazyCardDetailsDemo />

<Callout tone="essentials">
Other board-sized opportunities for code splitting and on-demand loading:

- **“Power tools” panel**: keep advanced filters, bulk actions, export, and reporting in separate chunks until the user opens them.
- **Drag-and-drop mode**: ship a lightweight list UI first; only load the DnD library and adapters when drag mode is enabled.
- **Virtualisation on scale**: lazy-load list machinery when you add windowing; scope note in the **aside** below.
- **Permission-gated admin surfaces**: never ship audit log / permissions editors to viewers.
- **“Bundle story” overlay**: a teaching/debug panel that lists newly loaded scripts after a click (so the article can *prove* what loaded).
</Callout>
...
<Callout tone="aside">
**Virtualisation, pagination, caching, and friends** — list **virtualisation** (windowing columns or feeds), **cursor-based pagination** (we sketched reads for huge lanes in **API design**), **client-side caching** (SWR-style staleness, normalised stores, prefetch), and related read-path tricks are core tools for frontend performance at scale. On many *board-shaped* products you can ship a long way with a snapshot, code-splitting, and deliberate loading states first; those heavier techniques become unavoidable when columns turn into **thousands** of cards or the surface behaves more like a **timeline or inbox** than a compact kanban.

This walkthrough stays intentionally bounded so it does not become a single “god article.” Deeper treatment of those patterns belongs in **other case studies** (and pattern pages) on the site — browse the [case studies index](/case-studies) as they land — while this board piece keeps its spine on **shape, contracts, collaboration, and ordering**.
</Callout>
...
<Callout tone="essentials">
For **system design**, the connection is straightforward: the decisions you argue for—**idempotent** moves, **versioned** snapshots, **SSE** vs poll, **LexoRank** vs integers—are only as real as the code that honours them. Tests turn those claims into **checkable behaviour** (“given this event order, the client model looks like this”), which is how you de-risk a design when you move from the whiteboard to implementation—or when an interviewer asks how you would **verify** your approach.
</Callout>
